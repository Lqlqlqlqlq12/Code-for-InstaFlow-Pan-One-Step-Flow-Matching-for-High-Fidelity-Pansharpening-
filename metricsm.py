...
def q2n_torch(
    ...
    block: int = 32,
    eps: float = 1e-12,
    channels_first: bool = True,     # True: [B,C,H,W] 或 [C,H,W]；False: [B,H,W,C] 或 [H,W,C]
    pad_to_pow2_channels: bool = True,
    reduction: str = "mean",         # "mean" -> 标量；"none" -> 返回每张图的 Q2n（B,）
    return_patch_scores: bool = False
) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    """
    Q2n（多光谱 UIQI）计算。返回值：
      - reduction="mean"          -> 标量
      - reduction="none"          -> [B]，每张图一个分数
      - 若 return_patch_scores    -> 额外返回 [B, n_patches] 的分块分数
    说明：
      - 使用非重叠窗口；若 H/W 不是 block 的整数倍，将**丢弃边缘**（常见做法）。
      - 若 pad_to_pow2_channels=True，会在通道维 0 填充到最近的 2^n（两图一致，不影响结果）。
    """
    assert ref.shape == test.shape, "ref/test 形状需一致"
    x = ref
    y = test
    added_batch = False
    ...
    if channels_first:
        if x.dim() == 3:
            x = x.unsqueeze(0); y = y.unsqueeze(0); added_batch = True
        assert x.dim() == 4, "期望 [B,C,H,W] 或 [C,H,W]"
        B, C, H, W = x.shape
    else:
        if x.dim() == 3:
            # [H,W,C] -> [1,C,H,W]
            x = x.permute(2,0,1).unsqueeze(0); y = y.permute(2,0,1).unsqueeze(0); added_batch = True
        elif x.dim() == 4:
            # [B,H,W,C] -> [B,C,H,W]
            x = x.permute(0,3,1,2); y = y.permute(0,3,1,2)
        else:
            raise ValueError("期望 [H,W,C] / [B,H,W,C]")
        B, C, H, W = x.shape
    ...
    # 可选：通道补到 2^n
    if pad_to_pow2_channels:
        pow2 = 1 << int(torch.ceil(torch.log2(torch.tensor(float(C)))).item())
        if pow2 != C:
            padC = pow2 - C
            z = torch.zeros((B, padC, H, W), dtype=dtype, device=device)
            x = torch.cat([x, z], dim=1)
            y = torch.cat([y, z], dim=1)
            C = pow2

    # 丢弃边缘，使 H,W 为 block 的整数倍
    Hc = (H // block) * block
    Wc = (W // block) * block
    x = x[:, :, :Hc, :Wc]
    y = y[:, :, :Hc, :Wc]

    # 提取非重叠块：unfold -> [B, C*P, L]，P=block*block, L=块数
    P = block * block
    X = F.unfold(x, kernel_size=block, stride=block)    # [B, C*P, L]
    Y = F.unfold(y, kernel_size=block, stride=block)    # [B, C*P, L]

    # 变成 [B, L, P, C]
    Lnum = X.shape[-1]
    X = X.view(B, C, P, Lnum).permute(0, 3, 2, 1).contiguous()
    Y = Y.view(B, C, P, Lnum).permute(0, 3, 2, 1).contiguous()

    # 均值（向量均值，跨像素 P）
    mu_x = X.mean(dim=2)                               # [B, L, C]
    mu_y = Y.mean(dim=2)                               # [B, L, C]

    # 去均值
    Xc = X - mu_x.unsqueeze(2)                         # [B, L, P, C]
    Yc = Y - mu_y.unsqueeze(2)

    # 方差项（向量范数平方，先 sum_C 再 mean_P）
    s_x2 = (Xc * Xc).sum(dim=3).mean(dim=2)            # [B, L]
    s_y2 = (Yc * Yc).sum(dim=3).mean(dim=2)            # [B, L]

    # 互相关项（向量点积，再 mean_P）
    c_xy = (Xc * Yc).sum(dim=3).mean(dim=2)            # [B, L]

    # 三个分量：结构（相关系数绝对值）、对比、亮度
    denom = torch.sqrt(s_x2 * s_y2) + eps
    rho   = c_xy / denom                               # [B, L]
    Cterm = (2.0 * denom) / (s_x2 + s_y2 + eps)        # [B, L]

    nmu_x = torch.linalg.vector_norm(mu_x, dim=2)      # [B, L]
    nmu_y = torch.linalg.vector_norm(mu_y, dim=2)      # [B, L]
    Lterm = (2.0 * nmu_x * nmu_y) / (nmu_x**2 + nmu_y**2 + eps)

    q_patch = torch.abs(rho) * Cterm * Lterm           # [B, L]
    q_img   = q_patch.mean(dim=1)                      # [B]
    ...
    if reduction == "mean":
        ...
        raise ValueError("reduction 仅支持 'mean' 或 'none'")

    if added_batch and reduction == "none":
        # 调用了单图输入 + reduction='none'，返回标量更方便
        out = out.squeeze(0)
